...
# -*- coding: utf-8 -*-
"""
Created on Sat Jan 11 15:49:51 2020
...
class Metrics:
    ...
    def make_citation_list(self, geo_id, pmids):
        citations = self.geo_citation_dict[geo_id]
        result_list = []
        if not len(citations) == 0:
            present = list(set(citations).intersection(set(pmids))) #first citations should be the one we calculate to have the highest similarity
            if len(present) == 0:
    ...
    def calculate_recall_at_k(self, similarity_dict, k):
        ...
        for geo_id in similarity_dict:
            selected_pmids_dict = similarity_dict[geo_id] #pmids with values for a particular geo_id
            result_list = self.make_citation_list(geo_id, list(selected_pmids_dict.keys()))
            citations_len = len(self.geo_citation_dict[geo_id])
            if not len(result_list) == 0: 
                final_result_list.append(result_list)
