...
def main():
    ...
    with (open("../IIdata/srastudiesPubs.ls", "rb")) as openfile:
        pubs2scrap = pickle.load(openfile)
    ''' with (open("../IIdata/srastudies.pickle", "rb")) as openfile:
        pubs = pickle.load(openfile)'''
        
    ##only scrap those not in dataset yet
    #pubs_exi = list(pubs.keys())
    #pubs2scrap = np.setdiff1d(pubs2scrap,pubs_exi)
    
    ##not to save in one lump, do it in batches 
    #add into the dictionary 
    count = 0
    #count = 60000
    every = 10000 #this value can be changed 
    new_pubs_dict ={}
    for pub in pubs2scrap:#[60000:]:
        ...
        if count % every == 0:
            with open('../IIdata/scraped_srastudiespubs_dict_' + str(count) + '.pickle', 'wb') as handle:
                    pickle.dump(new_pubs_dict, handle, protocol=pickle.HIGHEST_PROTOCOL)
   
    #print one, dump all 
    with open('../IIdata/scraped_srastudiespubs_dict.pickle', 'wb') as handle:
        pickle.dump(new_pubs_dict, handle, protocol=pickle.HIGHEST_PROTOCOL)
    ...
    ##merge all scraped files together, uncomment this when you have everthing scraped
    '''
    import glob
    filelist = glob.glob('../data/scraped_pubs_dict*.pickle')
    pub_dataset = {}  # Create an empty dictionary

    for i in range(len(filelist)):
        with open(filelist[i], 'rb') as f:
            pub_dataset.update(pickle.load(f)) 
    print(len(pub_dataset))
    with open('../data/pub_dataset.pickle', 'wb') as handle:
        pickle.dump(pub_dataset, handle, protocol=pickle.HIGHEST_PROTOCOL)  #73,248 
    '''  
