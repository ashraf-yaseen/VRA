...
class dataPreProcess:
    def __init__(self,geopath = 'GEOMetaDataCollection/geo_dataset.pickle', output = 'data/'):
        self.geopath = geopath 
        self.geodata = pickle.load(open(self.geopath, 'rb'))
        self.output = output
        self.citation = None
        self.publs = None
    ...
    def load_citation_ls(self):
        if os.path.exists(self.output + 'citation_data.dict') and \
        os.path.exists(self.output + 'pubsIncitation.ls') and \
        os.path.exists(self.output + 'geosIncitation.ls'):
            self.citation = pickle.load(open(self.output + 'citation_data.dict', 'rb'))
            self.publs = pickle.load(open(self.output + 'pubsIncitation.ls', 'rb'))
            self.geols = pickle.load(open(self.output + 'geosIncitation.ls', 'rb'))
        else:
    ...
    def get_geocitation_notnull(self):
        ...
        citation_dict = {}
        counter = 0 
        for k,v in self.geodata.items():
            if (len(v['citations']) !=  0) and (v['citations'][0].strip() != ''):
                citation_dict[k] = v['citations']
        self.citation = citation_dict 
        with open(self.output + 'citation_data.dict', 'wb') as handle:
            pickle.dump(self.citation, handle, protocol=pickle.HIGHEST_PROTOCOL)
        self.keys_fromcitation(name = 'geos')
    
        #sanity check
        print('total length:', len(self.citation))
        print('sample data structure:')
        self.get_item(self_citation)
    ...
    def reverse_citation(self):
        ...
        for k,v in self.citation.items():
            papls = len(v)
            for i in range(papls): 
                if v[i] in new_citation_data.keys():                      
                      new_citation_data[v[i]].append(k) 
                else:
                    new_citation_data[v[i]] = [k]
        self.citation = dict(new_citation_data)
        with open(self.output + 'citation_data.dict', 'wb') as handle:
            pickle.dump(self.citation, handle, protocol=pickle.HIGHEST_PROTOCOL)
        self.keys_fromcitation(self, name = 'pubs')
    ...
    def cut_citation(self, perc = 0.999):
        ...
        with open(self.output + 'citation_data.dict', 'wb') as handle:
            pickle.dump(self.citation, handle, protocol=pickle.HIGHEST_PROTOCOL)
        self.keys_fromcitation(self, name='pubsCut')
    ...
    def create_pairs(self, ratio = 1, every = 100, error = 10):
        ...
        if os.path.exists(self.output + 'pubsIncitation.ls') and \
        os.path.exists(self.output + 'geosIncitation.ls'):
            self.publs = pickle.load(open(self.output + 'pubsIncitation.ls', 'rb'))
            self.geols = pickle.load(open(self.output + 'geosIncitation.ls', 'rb'))
        else:
        ...
        with open(self.output + 'true_pairs.pkl', 'wb') as handle:
        ...
        random.seed(1234)
        final = []
        chunks = (len(self.geols) - 1) // every + 1
        for i in range(chunks):
            batch = geols[i*every:(i+1)*every]
            #batch permutation 
        ...
        with open(self.output+'random_false_pairs.pkl', 'wb') as handle:
