...
def train_batch2(Batch, model, optimizer, scheduler, cuda_device):
...
def train_batch(Batch, dp2, model, optimizer, scheduler, cuda_device):
    ...
    #   [3]:labels
    #   [4]:tfidf weights
    # we need to use dataprocessing tensorizeAB here
    # step 1 get tensorized targets
    b_labels = torch.tensor(Batch[2]).cuda(cuda_device)
    b_input_ids, b_input_mask, b_input_type_ids = dp2.tensorize_AB(Batch[0], Batch[1])
    

    b_input_ids = b_input_ids.cuda(cuda_device)
    b_input_mask = b_input_mask.cuda(cuda_device)
    b_input_type_ids = b_input_type_ids.cuda(cuda_device)
    
    #b_input_ids = Batch[0].cuda(cuda_device)
    #b_input_mask = Batch[1].cuda(cuda_device)
    #b_input_type_ids = Batch[2].cuda(cuda_device)
    #b_labels = Batch[3].cuda(cuda_device)
    #we need to first tensorize then do the cuda and then 
...
def train(epochs,dp2, model, train_loader, valid_loader, optimizer, scheduler, use_cuda, args):
...
def evaluate_batch2(Batch, model, cuda_device):
...
def evaluate_batch(Batch, dp2, model, cuda_device):
        b_labels = torch.tensor(Batch[2]).cuda(cuda_device)
        b_input_ids, b_input_mask, b_input_type_ids = dp2.tensorize_AB(Batch[0], Batch[1]) 
...
def predictions(dp2, model, test_loader, use_cuda, path):
