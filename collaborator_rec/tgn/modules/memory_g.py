#let's keep the memory module at CPU all the time 
#only moves to GPU when needed
...
class Memory(nn.Module):
  ...
  def __init_memory__(self):
    ...
    self.last_update = nn.Parameter(torch.zeros(self.n_nodes)#.to(self.device),
                                    requires_grad=False) #modified by ginny, we should use device whenver necessary only 
